...
const getGraphPrice = async () => {
  ...
  const sum = validHistories.reduce(
    (acc: number, cur: { closePriceDAI: number }) => acc + Number(cur.closePriceDAI),
    0
  );
...
const reportOracle = async (average: ethers.BigNumber, oracle: ethers.Contract) => {
  ...
  const lesserKey =
    insertIndex >= 0 && keys[insertIndex].toLowerCase() != signer.toLowerCase()
      ? keys[insertIndex]
      : ethers.constants.AddressZero;
  const greaterIndex = values.findIndex((val) => val.gt(average));
  const greaterKey =
    greaterIndex >= 0 && keys[greaterIndex].toLowerCase() != signer.toLowerCase()
      ? keys[greaterIndex]
      : ethers.constants.AddressZero;
...
const getCeloPrice = async (cmcKey) => {
  ...
  if (cmcKey) {
    const { data } = await axios.get('https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=CELO', {
...
exports.handler = async function (event: AutotaskEvent) {
  try {
    ...
    const averages = (
      results.filter((_) => _.status === 'fulfilled') as Array<PromiseFulfilledResult<ethers.BigNumber>>
    ).map((_) => _.value);
